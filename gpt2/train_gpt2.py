...
class GPTConfig:
    ...
    vocab_size: int = 50257 # 50,000 BPT merges + 256 bytes toekns + 1 <|endoftext|> token
...
class GPT(nn.Module):
    ...
    def _init_weights(self, module):
        # following the gpt-2 source code
        if isinstance(module, nn.Linear):
            std = 0.02
            if hasattr(module, 'LLMC_RESIDUAL_SCALE_FLAG'):
                # control residual stream by multiplying 1/sqrt(n_layer)
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb # boardcasting of pos
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # i.e. all weight tensors in the matmuls + embeddings decay, all biases and layernorms don't.
...
B = 4 # micro batch size, bigger gpu can add up numbers
...
for step in range(max_steps):
    ...
    # determine and set the laerning rate for this iteration
    lr = get_lr(step)
    ...
    print(f"step {step} | loss: {loss_accum.item():.6f} | lr: {lr:.2e} | norm: {norm:.4f} | dt: {dt: .2f}ms | tok/sec: {tokens_per_sec:.2f}")
...
#model = GPT(GPTConfig())
model.eval() # whenever not training the model
model.to(device)
...
while x.size(1) < max_length:
    ...
    with torch.no_grad():
        ...
        # take the logits at the last position - inefficient!
